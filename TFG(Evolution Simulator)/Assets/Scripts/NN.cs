...
public class NN : MonoBehaviour
{
    ...
    public void Awake()
    {
        int hiddenNeurons = Random.Range(32, 65);//Dependiendo cual toque sera mas listo o menos, no es hereditario
        networkShape = new int[] { numInputs, hiddenNeurons, 2 };
    ...
    void Start()
    {
        // Si el prefab tiene un NN guardado con diferente tamaño de entrada,
        // esto forzará a regenerar la red neuronal correctamente.
        if (layers == null || layers.Length == 0 || layers[0].n_inputs != numInputs)
        {
            Debug.Log("Regenerando red neuronal con " + numInputs + " entradas.");
            Awake();
        }
    }
    ...
    public Layer[] copyLayers()
    {
        ...
        for(int i = 0; i < layers.Length; i++)
        {
            tmpLayers[i] = new Layer(layers[i].n_inputs, layers[i].n_neurons);
            // Copiar pesos
            System.Array.Copy(
                layers[i].weightsArray,
                tmpLayers[i].weightsArray,
                layers[i].weightsArray.Length
            );

            // Copiar biases
            System.Array.Copy(
                layers[i].biasesArray,
                tmpLayers[i].biasesArray,
                layers[i].biasesArray.Length
            );
        }
        return(tmpLayers);
    }
